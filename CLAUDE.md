...
## Stdlib — see [stdlib.md](./stdlib.md) for full reference
...
Key functions:
- `string`: trim split join len lines chars contains replace index_of starts_with? is_digit? ...
- `list`: get get_or len sort map filter find fold enumerate zip flatten take drop unique ...
- `map`: new get set contains remove keys values entries from_list ...
- `fs`: read_text write read_lines append mkdir_p exists? remove list_dir ...
- `regex`: match?(pat,s) full_match? find find_all replace split captures  (* use r"..." for patterns *)
- `json`: parse stringify get get_string get_int get_bool get_array keys ...
- `math`: min max abs pow pi sin cos sqrt ...
- `time`: now millis sleep year month day hour minute second weekday to_iso from_parts ...
- `random`: int(min,max) float choice shuffle ...
